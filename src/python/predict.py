...
test_features2 = test_features[test_features["private_profile"] == 2].drop(user_partial_drop, axis = 1)
test_features2["prediction"] = np.exp(model2.predict(test_features2)) -1

test_features3 = test_features[test_features["private_profile"] == 1].drop(user_full_features, axis = 1)
